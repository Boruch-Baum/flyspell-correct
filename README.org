...
This is a fork of the [flyspell-correct git
repository](https://github.com/d12frosted/flyspell-correct), providing
...
+ added feature to push the point onto the mark ring for each spelling
  correction, so one can review and navigate the buffer to corrected
  words after running the commands.

+ added "rapid" feature to automatically progress to every remaining
...
+ add convenience function 'flyspell-popup-wrapper' so users can
